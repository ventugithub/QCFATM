...
for (d, w) in itertools.product(delays, penalty_weights):
    for p in partitions:
        ...
        cmd = './runInstance.py --num_embed %i -i %s --retry_embedding 3  --retry_embedding_desperate 1 --exact --unary  --use_snapshots --inventory %s -p2 %f -p3 %f' % (num_embed, instancefiles[0], inventoryfile, w, w)
        print cmd
