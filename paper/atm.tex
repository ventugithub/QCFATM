...
As mentioned above the maneuvers enter our formulation as additional delays at the conflict time $d_{ik}$.
We want to reduce the problem make it amenable to a QUBO formulation.
In the course of this, however, we need to make sure to retain the combinatorial nature of the maneuver decision problem.
We do this by restricting the vast realm of maneuvers to two distinct choices:
Either one of the two involved flights is delayed while leaving the other flight untouched
...
\end{equation*}
where we have introduced a temporal ordering of the conflicts for each flight $i$ by
\begin{align*}
                 &k < k' \text{ if } t < t' \\
    \text{ for } &t = \arg \min_s x_{i, s} \in C_k \; , \\
                 &t' = \arg \min_s x_{i, s} \in C_{k'}
\end{align*}
...
\begin{equation*}
    \tilde Q_\text{delay} = \frac{1}{d_\text{max}}\sum_{i\alpha} \alpha d_{i\alpha}  + \sum_{ik} d_M \tilde a_{ik}\; ,
\end{equation*}
...
For unique representation of the variables we add the following contribution to the QUBO
\begin{align*}
