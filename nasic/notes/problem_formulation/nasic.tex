...
\begin{itemize}
    ...
    \item Constraint \eqref{eqn:constraint_airfield_total_resource} $\forall i$: $\sum_j x_{i,j} + \sum_{i', j'} y_{i, i', j'} \leq B(i)$
...
\begin{enumerate}
    ...
    \begin{align*}
        ...
        y_{i, i', j'} & = \sum_{\alpha = 0}^{D_i} 2^\alpha y_{i, i', j', \alpha} + (b(i) - 2^D_i + 1) \sum_{\alpha=0}^{b(i) - 2^D_i + 1} \frac{\alpha y_{i, i', j', D_i + \alpha}}{(b(i) - 2^D_i + 1)}
    \end{align*}
...
\begin{equation*}
    z^1_{i} = \sum_{\alpha = 0}^{D_i} 2^\alpha z_{i, \alpha}
\end{equation*}
QUBO contribution
\begin{equation*}
