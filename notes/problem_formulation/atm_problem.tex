...
In the following we will define spatial, potential an real conflicts.
A \textit{spatial conflict} occurs if two trajectories become to close to each other in space.
...
For detail, see appendix \ref{app:potential_conflicts}.

In any case, we denote the conflicts by $k$.
...
In general, delay introduced by conflict avoiding maneuver $d_{ik}$ is a function of the arrival delay of each involved flight $D_{jk}, \forall j \in I_k$ and the maneuver parameters $\mathbf{a_k}$
\begin{equation} \label{eqn:local_delay}
...
By sparsity, we mean that all potential conflicts are between only two trajectories and that no other trajectories are nearby in spacetime, so that potential maneuvers for avoiding a conflict between two flights do not introduce additional potential conflicts with other flights.\footnote{This could be extended to an $m$-sparse version, where each conflict can affect $m-1$ others.}

We assume that $d_{ik}$ is function of the difference of the delays 
\begin{equation*}
...
Discretize and introduce bits for each discrete value for the variables $d_i$, $d_{ik}$, $\Delta_k = D_{lk} - D_{jk}$, and $a_k$:
\begin{equation*}
...
&=
\sum_{k} \left[
\left(\sum_{\alpha} d_{l\alpha} + \sum_{p \in K^<_{lk}} \sum_{\beta} \beta d_{lp\beta}\right)
-
\left(\sum_{\alpha} d_{j\alpha} + \sum_{q \in K^<_{jk}} \sum_{\beta} \beta d_{jq\beta}\right)
-
